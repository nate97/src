...
class HolidayManagerAI:
    ...
    def switchClassicChars(self, holiday): # This is a horrible way todo this, but honestly who cares, I just want them to change!
        return
        defaultHoods = [2000, 3000, 4000, 5000, 8000, 9000]

        if holiday == HALLOWEEN_COSTUMES:
            specialHoods = [6000] # Halloween does something with zone 6000
        else:
            specialHoods = [0] # April fools day doesn't do anything with zone 6000

        for hood in self.air.hoods:
            zoneId = hood.zoneId
            if zoneId in defaultHoods:
                # Delete old
                hood.classicChar.requestDelete()

                # Generate new character
                hood.createClassicChar()

                # New character explode in
                hood.classicChar.fadeAway()

            elif zoneId in specialHoods:
                # Delete old
                hood.classicCharChip.requestDelete()
                hood.classicCharDale.requestDelete()

                # Generate new characters
                hood.c()

                # New characters explode in
                hood.classicCharChip.fadeAway()
                hood.classicCharDale.fadeAway()
            else:
                pass
