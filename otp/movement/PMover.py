...
class PMover:
    ...
    def __init__(self, objNodePath, fwdSpeed=1, rotSpeed=1):
        ...
        self.minDist = 5
        self.stopRotationFix = False
        self.stopRotationTrick = False
    ...
    def pauseRotation(self, pause):
        self.stopRotationFix = pause

    def petRotationFix(self):
        if self.stopRotationTrick != True:
            if self.stopRotationFix != True:

                if self.petMode == 'chase':
                    self.objNodePath.lookAt(self.target)
                    return

                elif self.petMode == 'static_chase':
                    self.objNodePath.lookAt(self.target)
                    return

                elif self.petMode != 'stick' or self.petLocked:
                    currentH = self.objNodePath.getH()
                    correctedH = currentH - 180
                    self.objNodePath.setH(correctedH)
                    return
